...
public class SensorDataBytesS extends SensorDataBytes{
	 ...
	 * 多例模式
	 */
	private static HashMap<String, SensorDataBytesS>  sensorDataBytesMap=new HashMap<String, SensorDataBytesS>(); 
	 ...
	 */
	private SensorDataBytesS(String name){
		try {
			System.out.println(name);
			numSizeTable=ReadSensorByteXml.readSensorByteXml2(name);
	...
	public static SensorDataBytes getInstance(String name){
		if(sensorDataBytesMap.get(name)==null){
			sensorDataBytesMap.put(name,new SensorDataBytesS(name));
		}
